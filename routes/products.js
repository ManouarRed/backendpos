...
const getImageUrl = (imagePath) => {
    ...
    return `${process.env.API_BASE_URL || 'http://192.168.3.192:3001'}${imagePath}`;
};

const getRelativeImagePath = (imageUrl) => {
    if (!imageUrl) return '';
    const baseUrl = process.env.API_BASE_URL || 'http://192.168.3.192:3001';
    if (imageUrl.startsWith(baseUrl)) {
        return imageUrl.substring(baseUrl.length);
    }
    return imageUrl; // Return as is if it's an external URL or already relative
};
...
async function enrichProductData(connection, productRow) {
    ...
    return {
        ...
        fullSizeImage: getImageUrl(productRow.full_size_image),
...
router.get('/admin', protect, async (req, res) => {
    ...
    try {
        ...
        const [productsRows] = await connection.execute(productsQuery, finalQueryParams.map(p => p.toString()));
...
router.post('/', protect, isAdmin, async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.execute(
            ...
            [newProductUuid, title.trim(), code.trim(), parseFloat(price), dbCategoryId, dbManufacturerId, getRelativeImagePath(image), getRelativeImagePath(fullSizeImage), isVisible !== undefined ? Boolean(isVisible) : true]
        );
...
router.put('/bulk-update', protect, isAdmin, async (req, res) => {
    const { productIds, updates } = req.body; // Expecting an array of product UUIDs and an updates object

    if (!Array.isArray(productIds) || productIds.length === 0 || !updates || Object.keys(updates).length === 0) {
        return res.status(400).json({ success: false, message: 'Invalid request body for bulk update.' });
    }
    ...
    try {
        ...
        if (updates.categoryId) {
            const [catRow] = await connection.execute('SELECT category_id FROM categories WHERE uuid = ?', [updates.categoryId]);
            if (catRow.length === 0) {
                await connection.rollback();
                return res.status(400).json({ message: "Invalid Category ID." });
            }
            updateFields.category_id = catRow[0].category_id;
        }

        if (updates.manufacturerId) {
            const [manRow] = await connection.execute('SELECT manufacturer_id FROM manufacturers WHERE uuid = ?', [updates.manufacturerId]);
            if (manRow.length === 0) {
                await connection.rollback();
                return res.status(400).json({ message: "Invalid Manufacturer ID." });
            }
            updateFields.manufacturer_id = manRow[0].manufacturer_id;
        }

        if (updates.isVisible !== undefined) {
            updateFields.isVisible = Boolean(updates.isVisible);
        }
        ...
        if (Object.keys(updateFields).length > 0) {
            ...
            if (internalProductIds.length > 0) {
                await connection.execute(
                    `UPDATE products SET ${setClauses} WHERE product_id IN (${internalProductIds.map(() => '?').join(',')})`,
                    [...updateValues, ...internalProductIds]
                );
            }
        }

        await connection.commit();
        res.json({ success: true, message: `${productIds.length} products updated successfully.` });

    } catch (error) {
        if (connection) await connection.rollback();
        console.error("Failed to bulk update products:", error);
        res.status(500).json({ success: false, message: "Failed to bulk update products: " + error.message });
    } finally {
        if (connection) connection.release();
    }
});
...
router.put('/:uuid', protect, isAdmin, async (req, res) => {
    ...
    try {
        ...
        if (image !== undefined) updateFields.image = getRelativeImagePath(image);
        if (fullSizeImage !== undefined) updateFields.full_size_image = getRelativeImagePath(fullSizeImage);
