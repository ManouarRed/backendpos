...
router.post('/', protect, async (req, res) => {
    const { items, totalAmount, paymentMethod, notes, discount } = req.body;
    ...
    try {
        ...
        const [saleResult] = await connection.execute(
            'INSERT INTO sales (uuid, user_id, total_amount, payment_method, notes, discount, submission_date) VALUES (?, ?, ?, ?, ?, ?, NOW())',
            [saleUuid, userId, parseFloat(totalAmount), paymentMethod, notes || '', parseFloat(discount) || 0]
        );
        ...
        for (const item of items) {
            ...
            if (item.isManual === true) {
                ...
                if (!item.productId) {
                    ...
                    return res.status(400).json({ message: `Product ID is missing for a non-manual item.` });
                }
...
router.put('/:uuid', protect, async (req, res) => {
    ...
    try {
        ...
        await connection.execute(
            'UPDATE sales SET total_amount = ?, payment_method = ?, notes = ?, discount = ?, updated_at = NOW() WHERE sale_id = ?',
            [parseFloat(totalAmount), paymentMethod, notes || '', parseFloat(discount) || 0, saleId]
        );

        await connection.execute('DELETE FROM sale_items WHERE sale_id = ?', [saleId]);

        for (const item of items) {
            console.log("DEBUG: Processing sale item for update:", item); // Added logging
            let dbProductId = null;
            let productUuid = null;
            ...
            if (item.isManual === true) {
                ...
                if (!item.productId) {
                    // This is a product from the database, but it's missing an ID.
                    // This can happen if the client-side state is not managed correctly.
                    // We will treat it as a manual entry with a warning.
                    console.warn('Item was not manual but was missing a productId. Treating as manual.', item);
                    dbProductId = null;
                    productUuid = null; // Ensure this is null for the INSERT
                } else {
                    const [productRow] = await connection.execute('SELECT product_id FROM products WHERE uuid = ?', [item.productId]);
                    if (productRow.length === 0) {
                        await connection.rollback();
            ...
            await connection.execute(
                `INSERT INTO sale_items (sale_id, product_id, product_uuid, title, code, image, full_size_image, selected_size, quantity, unit_price, discount, final_price)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [saleId, dbProductId, productUuid, item.title, item.code, item.image, item.fullSizeImage, item.selectedSize, item.quantity, item.unitPrice, item.discount, item.finalPrice]
            );
        }
